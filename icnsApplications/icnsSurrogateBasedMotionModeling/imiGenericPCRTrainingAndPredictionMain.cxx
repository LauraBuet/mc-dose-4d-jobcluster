/** \file imiGenericPCRTrainingAndPredictionMain.cxx
 *
...
void PrintHelp()
{
  std::cout << "\n";
  std::cout << "Usage:\n";
  std::cout << "imiGenericPCRTrainingAndPrediction ... \n\n";
...
{
  imiINFO( "===================================================" );
  imiINFO( "====    imiGenericPCRTrainingAndPrediction     ====" );
  imiINFO( "===================================================" );
  ...
  imiPCRMotionPrediction* predictionFilter = imiPCRMotionPrediction::New();
  ...
  else
  {
    ...
    {
      imiERROR( "PCR not implemented for regressor file ending: " << fileTypePattern );
      return -1;
    }
  }
  ...
  else
  {
    ...
    {
      imiERROR( "PCR not implemented for regressand file ending: " << fileTypePattern );
      return -1;
    }
  }
  ...
  if( loocv )
  {
    ...
    noOfComponents = loocvTest->PerformLOOCVPCR( 1, std::min( regressorTrainingMatrix.cols() - 2, regressorTrainingMatrix.rows() ) );
    imiINFO( "LOOCV: "<<noOfComponents<<" selected!" );

  }
  else
  {
    if( noOfComponents > regressorTrainingMatrix.cols() - 1 )
    {
      imiWARNING(
          "The number of eigenvectors specified exceeds the number of regressor observations-1 available. Setting number of eigenvectors to "<<regressorTrainingMatrix.cols()-1<<" (# of regressor observations-1)!" );
  ...
  std::cout << std::endl;
  imiINFO( "Training PCR system matrix ... " );
  ...
  for( unsigned int i = 0; i < noOfInputObservations; i++ )
  {
    ...
    imiINFO( "\n------------------------------------------" );
    imiINFO( "imiGenericPCRTraining finished." );
    imiINFO( "==========================================\n" );
